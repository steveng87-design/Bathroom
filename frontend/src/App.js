...
Generated by Bathroom Quote Saver.AI - Professional AI-Powered Quoting System`;
                      ...
                      <Button
                        onClick={async () => {
                          try {
                            ...
                            // Clear current quote to show form
                            setQuote(null);
                            setAdjustedCosts({});
                            setAdjustmentMode(false);
                            
                            // Load the project data into the form
                            if (projectData.request_data) {
                              setFormData({
                                clientInfo: projectData.request_data.client_info || {
                                  name: '',
                                  email: '',
                                  phone: '',
                                  address: ''
                                },
                                roomMeasurements: projectData.request_data.room_measurements || {
                                  length: '',
                                  width: '',
                                  height: ''
                                },
                                components: projectData.request_data.components || formData.components,
                                additionalNotes: projectData.request_data.additional_notes || ''
                              });
                              
                              // Load task options
                              if (projectData.request_data.task_options) {
                                setTaskOptions(projectData.request_data.task_options);
                              }
                              
                              // Auto-expand components that were previously enabled so user can see/modify them
                              const enabledComponents = {};
                              if (projectData.request_data.components) {
                                Object.keys(projectData.request_data.components).forEach(componentKey => {
                                  if (projectData.request_data.components[componentKey]?.enabled) {
                              ...
                              console.log('Loaded project components:', projectData.request_data.components);
  ...
  const renderQuoteForm = () => {
    // If there's already a quote, show the quote results instead
    if (quote) {
      return (
        <div className="space-y-6">
          {/* Quote Results Display - This will show the generated quote */}
          <div className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
            ...
            <div className="text-center py-8">
              <p className="text-xl text-gray-600">Quote results would be displayed here</p>
              <p className="text-gray-500 mt-2">Full quote display integration in progress...</p>
            </div>
          </div>
        </div>
      );
                        ...
                        <div className="ml-6 space-y-2 bg-white/40 p-4 rounded-lg border border-purple-100">
                          ...
                          <div className="space-y-2">
                            {Object.entries(subtaskLabels[key] || {}).map(([subtaskKey, subtaskLabel]) => (
                              <div key={subtaskKey} className="flex items-start space-x-2">
                                <Checkbox
                                  id={`${key}_${subtaskKey}`}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 relative overflow-hidden">
      ...
      {/* Saved Projects Side Panel (Legacy - for sheet functionality) */}
