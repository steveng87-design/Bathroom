            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-8 bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-2xl border border-purple-100">
                  ...
                  <div className="space-y-4">
                    {/* TEMPORARY: Simplified component list to isolate infinite loop */}
                    <div className="border rounded-xl p-4 bg-white">
                      <p>Component selection temporarily disabled for debugging</p>
                      <p>Demolition: {currentArea?.components?.demolition?.enabled ? 'Enabled' : 'Disabled'}</p>
                      <p>Framing: {currentArea?.components?.framing?.enabled ? 'Enabled' : 'Disabled'}</p>
                      <button onClick={() => handleComponentToggle('demolition', true)}>
                        Enable Demolition (Test)
                      </button>
                    </div>
                    {/* 
                    {Object.entries(componentLabels).map(([key, label]) => (
