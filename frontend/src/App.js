...
const RenovationQuotingApp = () => {
  ...
  const handleFormSubtaskToggle = (component, subtask, enabled) => {
    // Update the current area's component subtasks instead of shared formData.components
    setProjectAreas(prev => 
      prev.map((area, index) => 
        index === currentAreaIndex 
          ? {
  ...
  // Helper function to calculate room perimeter for skirt tiles (in linear meters) - Memoized
  const calculatePerimeter = React.useMemo(() => {
    return (areaIndex = null) => {
      const area = areaIndex !== null ? projectAreas[areaIndex] : getCurrentArea();
      const { length, width } = area?.measurements || {};
...
Generated by Bathroom Quote Saver.AI - Professional AI-Powered Quoting System`;
  ...
  const loadProject = async (projectId) => {
    try {
      ...
      // Navigate to New Quote view to show loaded project
      setCurrentView('new-quote');
      
      toast.success(`Project loaded successfully: ${project.project_name}`);
      console.log('Project loaded and navigated to New Quote view');
      ...
      if (error.response?.status === 404) {
        ...
        errorMessage = 'Server error loading project - please try again';
      } else if (error.code === 'NETWORK_ERROR') {
        errorMessage = 'Network error - please check your connection';
      } else if (error.response?.data?.detail) {
        errorMessage = `Failed to load project: ${error.response.data.detail}`;
      }
  ...
  // Calculations now update naturally through React's normal re-render cycle
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6">
              {/* Quote Actions - TOP ROW: Only Edit Button */}
              <div className="flex gap-2 mb-8">
                <Button 
                  onClick={() => setCurrentView('new-quote')}
                  variant="outline"
                  className="border-blue-600 text-blue-600 hover:bg-blue-50"
                >
                  <Edit3 className="w-4 h-4 mr-2" />
                  Edit Tasks & Components
                </Button>
              </div>
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6">
              ...
              <div className="border-t pt-4 mt-6">
                ...
                {(Object.keys(adjustedCosts).length > 0 || (quote && quote.cost_breakdown && quote.cost_breakdown.some(item => item.adjusted_cost !== undefined))) && (
          ...
          {/* Duplicate action buttons removed - using clean bottom action bar instead */}
        </div>
      );
    }
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-blue-800">Currently Editing: {currentArea?.name}</h3>
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-6 bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-2xl border border-purple-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Component selection temporarily disabled to fix infinite loop */}
                  <div className="col-span-2 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                    <p className="text-amber-800 font-medium">⚠️ Component selection temporarily simplified</p>
                    <p className="text-amber-700 text-sm">The detailed component selection interface has been temporarily disabled due to a technical issue. Use the simplified version in the next section.</p>
                  </div>
                    {/* ALL COMPONENT MAPPING CODE COMMENTED OUT TO PREVENT INFINITE LOOP
                    <div key={key} className="space-y-3">
                      <div className="flex items-center space-x-4 p-6 bg-white/60 rounded-xl border border-purple-200/50 hover:bg-white/80 transition-all cursor-pointer" 
                           onClick={() => handleFormComponentToggle(key, !currentArea?.components?.[key]?.enabled)}>
                        <Checkbox
                          id={key}
                          checked={currentArea?.components?.[key]?.enabled || false}
                          onCheckedChange={(checked) => handleFormComponentToggle(key, checked)}
                          className="h-6 w-6 data-[state=checked]:bg-purple-600 data-[state=checked]:border-purple-600 cursor-pointer"
                        />
                        <Label htmlFor={key} className="text-xl font-bold text-gray-800 cursor-pointer flex-1 leading-7">
                          {label}
                        </Label>
                        {currentArea?.components?.[key]?.enabled && (
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation(); // Prevent triggering the parent click
                              toggleExpandedComponent(key);
                            }}
                      ...
                      {currentArea?.components?.[key]?.enabled && expandedComponents[key] && (
                        <div className="ml-6 space-y-3 bg-white/50 p-5 rounded-xl border border-purple-100">
                          <p className="text-base text-purple-700 font-bold mb-4">Select specific {label.toLowerCase()} tasks:</p>
                          <div className="space-y-3">
                            {Object.entries(subtaskLabels[key] || {}).map(([subtaskKey, subtaskLabel]) => (
                              <div key={subtaskKey} className="space-y-3">
                                <div className="flex items-start space-x-4 p-3 bg-white/60 rounded-lg hover:bg-white/80 transition-all cursor-pointer"
                                     onClick={() => handleFormSubtaskToggle(key, subtaskKey, !currentArea?.components?.[key]?.subtasks?.[subtaskKey])}>
                                  <Checkbox
                                    id={`${key}_${subtaskKey}`}
                                    checked={currentArea?.components?.[key]?.subtasks?.[subtaskKey] || false}
                                    onCheckedChange={(checked) => handleFormSubtaskToggle(key, subtaskKey, checked)}
                                    className="mt-1 h-5 w-5 data-[state=checked]:bg-purple-500 cursor-pointer"
                                  />
                                  <Label htmlFor={`${key}_${subtaskKey}`} className="text-base text-gray-700 cursor-pointer flex-1 leading-6 font-medium">
                                    {subtaskLabel}
                                  </Label>
                                </div>
                                
                                {/* Task Options Dropdowns - Show when subtask is selected */}
                                {currentArea?.components?.[key]?.subtasks?.[subtaskKey] && (
                  ...
                  END OF COMMENTED OUT COMPONENT MAPPING CODE */}
                </div>
              </div>
  ...
  // Calculate current area directly to prevent infinite re-renders
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-semibold text-blue-800">
                    Currently Editing: {currentArea?.name}
                  </h4>
                  <div className="text-sm text-blue-600 flex items-center space-x-4 mt-1">
                    ...
                    {currentArea?.quote && (
                      <span className="font-semibold text-green-600">
                        Cost: ${currentArea?.quote?.total_cost?.toLocaleString()}
                      </span>
                    )}
                  </div>
                </div>
        ...
        {!quote ? (
            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-8 bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-2xl border border-purple-100">
                  ...
                  <div className="space-y-4">
                    {/* Component selection - original complex version causes infinite loop */}
                    <div className="border rounded-xl p-4 bg-white">
                      <h3 className="text-lg font-semibold mb-4">Renovation Components</h3>
                      <p className="text-sm text-amber-600 mb-4">
                        ⚠️ Component selection temporarily simplified due to technical issue
                      </p>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <button 
                            onClick={() => handleComponentToggle('demolition', !currentArea?.components?.demolition?.enabled)}
                            className="px-3 py-1 text-sm bg-blue-500 text-white rounded"
                          >
                            {currentArea?.components?.demolition?.enabled ? 'Disable' : 'Enable'}
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          <span>Framing</span>
                          <button 
                            onClick={() => handleComponentToggle('framing', !currentArea?.components?.framing?.enabled)}
                            className="px-3 py-1 text-sm bg-blue-500 text-white rounded"
                          >
                            {currentArea?.components?.framing?.enabled ? 'Disable' : 'Enable'}
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          <span>Plumbing</span>
                          <button 
                            onClick={() => handleComponentToggle('plumbing_rough_in', !currentArea?.components?.plumbing_rough_in?.enabled)}
                            className="px-3 py-1 text-sm bg-blue-500 text-white rounded"
                          >
                            {currentArea?.components?.plumbing_rough_in?.enabled ? 'Disable' : 'Enable'}
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <button 
                            onClick={() => handleComponentToggle('electrical_rough_in', !currentArea?.components?.electrical_rough_in?.enabled)}
                            className="px-3 py-1 text-sm bg-blue-500 text-white rounded"
                          >
                            {currentArea?.components?.electrical_rough_in?.enabled ? 'Disable' : 'Enable'}
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* End of simplified component section */}
                  </div>
                </div>
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            <Card className="shadow-lg">
              ...
              <CardContent className="p-6">
                {quote.area_quotes ? (
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                      <div className="text-center bg-gradient-to-br from-green-50 to-emerald-50 p-6 rounded-xl border border-green-200">
                        <h3 className="text-4xl font-black text-green-600">
                          ${quote?.total_project_cost?.toLocaleString() || quote?.total_cost?.toLocaleString() || '0'}
                        </h3>
                        <p className="text-green-700 font-semibold">Total Project Cost</p>
                    ...
                    {/* Individual Area Breakdowns - Conditional Display */}
                    <div className="space-y-6">
                      <h3 className="text-xl font-bold text-gray-900 flex items-center">
                        <Building className="w-5 h-5 mr-2 text-purple-600" />
                        {currentAreaView === 'combined' ? 'All Area Quotes' : 
                         currentAreaView === 'main' || currentAreaView === 'area1' ? 'Main Bathroom Quote' : 
                         'Separate WC Quote'}
                      </h3>
                      
                      {/* Filter areas based on current view */}
                      {quote.area_quotes
                        .filter(areaQuote => {
                          if (currentAreaView === 'combined') return true;
                          if (currentAreaView === 'main' || currentAreaView === 'area1') {
                            return areaQuote.area_name.toLowerCase().includes('main') || 
                                   areaQuote.area_name.toLowerCase().includes('bathroom');
                          }
                          if (currentAreaView === 'separate') {
                            return areaQuote.area_name.toLowerCase().includes('separate') || 
                                   areaQuote.area_name.toLowerCase().includes('toilet') ||
                                   areaQuote.area_name.toLowerCase().includes('wc');
                          }
                          return true;
                        })
                        .map((areaQuote, index) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-6">
                  ...
                  {/* Other action buttons moved to bottom action bar */}
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            {/* Duplicate Cost Breakdown removed - already exists in proper conditional above */}
