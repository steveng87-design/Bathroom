                  ...
                  {Object.entries(componentLabels).map(([key, label]) => (
                    <div key={key} className="space-y-3">
                      <div className="flex items-center space-x-4 p-6 bg-white/60 rounded-xl border border-purple-200/50 hover:bg-white/80 transition-all cursor-pointer" 
                           onClick={() => handleFormComponentToggle(key, !getCurrentArea()?.components?.[key]?.enabled)}>
                        <Checkbox
                          id={key}
                            ...
                            {Object.entries(subtaskLabels[key] || {}).map(([subtaskKey, subtaskLabel]) => (
                              <div key={subtaskKey} className="space-y-3">
                                <div className="flex items-start space-x-4 p-3 bg-white/60 rounded-lg hover:bg-white/80 transition-all cursor-pointer"
                                     onClick={() => handleFormSubtaskToggle(key, subtaskKey, !getCurrentArea()?.components?.[key]?.subtasks?.[subtaskKey])}>
                                  <Checkbox
                                    id={`${key}_${subtaskKey}`}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        ...
        {!quote ? (
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <div className="bg-blue-50 px-3 py-1 rounded">
                        <span className="text-sm text-blue-700">
                          New Total: <span className="font-bold">${formatCurrency(getTotalAdjustedCost())}</span>
                        </span>
                        <span className="text-xs text-blue-600 ml-2">
                          (Original: ${formatCurrency(quote.total_cost)})
                        </span>
                      </div>
                    </div>
                          ...
                          {adjustmentMode ? (
                            <div className="flex items-center gap-2">
                              ...
                              <Input
                                type="text"
                                placeholder="0"
                                value={adjustedCosts[index] !== undefined ? (adjustedCosts[index] === '' ? '' : adjustedCosts[index].toString()) : item.estimated_cost.toString()}
                                onChange={(e) => {
                                  let value = e.target.value;
                                  
                                  // Allow empty field
                                  if (value === '') {
                                    handleAdjustCost(index, '');
                                    return;
                                  }
                                ...
                                onBlur={(e) => {
                                  ...
                                  if (value !== '' && !isNaN(parseFloat(value))) {
                                    const formatted = parseFloat(value).toFixed(2);
                                    handleAdjustCost(index, formatted);
                                  }
                                }}
          ...
          {/* BOTTOM ACTION BAR - Only show when quote exists */}
          {quote && (
              ...
              <div className="max-w-6xl mx-auto">
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <Button
                    ...
                    className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white flex items-center justify-center py-3"
                  >
                  ...
                  <Button 
                    onClick={async () => {
                      try {
                        ...
                        toast.success('Quote saved to projects!');
                      } catch (error) {
                        console.error('Error saving quote:', error);
                        toast.error('Failed to save quote');
                      }
                    }}
                    className="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white flex items-center justify-center py-3"
                  >
                    <Save className="w-4 h-4 mr-2" />
                    Save Quote
                  </Button>
                  ...
                  <Button 
                    ...
                    className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white flex items-center justify-center py-3"
                  >
                  ...
                  <Button 
                    onClick={() => {
                      ...
                      setAdjustedCosts({});
                      setAdjustmentMode(false);
                      setCurrentAreaView('combined');
                      toast.success('Ready for new quote!');
                    }}
                    className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white flex items-center justify-center py-3"
                  >
                    <PlusCircle className="w-4 h-4 mr-2" />
                    New Quote
                  </Button>
      ...
      {/* Dialogs - Outside main content but inside component */}
