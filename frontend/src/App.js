          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6">
              ...
              <div className="border-t pt-4 mt-6">
                ...
                {Object.keys(adjustedCosts).length > 0 && (
                  <div className="text-sm text-gray-600 mt-1">
                    Original: ${quote.total_cost?.toLocaleString()} 
                    <span className={`ml-2 ${getTotalAdjustedCost() > quote.total_cost ? 'text-red-600' : 'text-green-600'}`}>
                      ({getTotalAdjustedCost() > quote.total_cost ? '+' : ''}${(getTotalAdjustedCost() - quote.total_cost).toLocaleString()})
                    </span>
                  </div>
                )}
              </div>
