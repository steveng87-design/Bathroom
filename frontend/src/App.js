          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6">
              ...
              <div className="flex gap-2 mb-8">
                ...
                {/* Area View Selector - only show if multi-area */}
                {(quote.areas_count > 1 || (quote.area_quotes && quote.area_quotes.length > 1)) && (
                  <Button
                    onClick={() => {
                      console.log('Area switch clicked. Current view:', currentAreaView);
                      if (currentAreaView === 'combined') {
                        setCurrentAreaView('main');
                      } else if (currentAreaView === 'main') {
                        setCurrentAreaView('separate');
                      } else {
                  ...
                  >
                    ...
                    {currentAreaView === 'combined' ? 'View Main Bathroom' : 
                     currentAreaView === 'main' ? 'View Separate WC' : 
                     'View Combined'}
                  </Button>
                )}
                ...
                <Button 
                  onClick={async () => {
                    try {
                      // Save current quote to projects using existing saveCurrentProject function
                      await saveCurrentProject();
                    } catch (error) {
                      console.error('Error saving quote:', error);
                      toast.error('Failed to save quote to projects');
                    }
                  }}
