...
const RenovationQuotingApp = () => {
  ...
  // Send Quote Email function - Updated to use native email client
  const handleSendQuoteEmail = async () => {
    ...
    try {
      ...
      // Always generate scope of works PDF if requested
      if (emailOptions.includePdf) {
        pdfsToGenerate.push('scope');
      }
      
      // Generate quote summary PDF
      pdfsToGenerate.push('quote');
      
      // Generate PDFs
      const pdfPromises = pdfsToGenerate.map(async (type) => {
        try {
          const endpoint = type === 'scope' 
            ? `${API}/quotes/${quote.id}/generate-proposal`
            : `${API}/quotes/${quote.id}/generate-quote-summary`;
            
          const response = await axios.post(endpoint, userProfile, {
            responseType: 'blob'
          });
          
          const filename = type === 'scope'
            ? `Scope_of_Works_${formData.clientInfo.name.replace(/\s+/g, '_')}_${quote.id.substring(0, 8)}.pdf`
            : `Quote_Summary_${formData.clientInfo.name.replace(/\s+/g, '_')}_${quote.id.substring(0, 8)}.pdf`;
          ...
          return { type, filename, success: true };
        } catch (error) {
          console.error(`Error generating ${type} PDF:`, error);
          return { type, filename: '', success: false };
        }
      });
      
      const results = await Promise.all(pdfPromises);
      const successfulPdfs = results.filter(r => r.success);
...
`;
      ...
      successfulPdfs.forEach(pdf => {
        const docType = pdf.type === 'scope' ? '📋 Detailed Scope of Works' : '💰 Quote Summary';
        emailBody += `${docType} (${pdf.filename})
`;
      });
...
Generated by Bathroom Quote Saver.AI - Professional AI-Powered Quoting System`;
      ...
      toast.success(
        `Email template opened in your default email app! ${successfulPdfs.length} PDF${successfulPdfs.length !== 1 ? 's' : ''} downloaded. Please attach ${successfulPdfs.length > 1 ? 'them' : 'it'} to your email before sending.`
      );
                      ...
                      <div className="space-y-4 py-4">
                        ...
                        <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                          <p className="text-sm text-blue-800">
                            <strong>📧 What happens:</strong> Opens your email app (iOS Mail, Gmail, etc.) with pre-written professional message. 
                            {emailOptions.includePdf ? '2 PDFs' : '1 PDF'} will download for you to attach manually.
                          </p>
                        </div>
