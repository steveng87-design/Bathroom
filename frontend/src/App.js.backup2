...
Generated by Bathroom Quote Saver.AI - Professional AI-Powered Quoting System`;
  ...
  const renderContractsView = () => {
    ...
    const generateStandaloneContract = async () => {
      ...
      try {
        // Create a temporary quote structure for contract generation
        const tempQuote = {
          id: `temp_${Date.now()}`,
          client_info: {
          ...
          total_cost: parseFloat(contractForm.totalPrice),
          cost_breakdown: [
            {
              component: 'Bathroom Renovation',
              estimated_cost: parseFloat(contractForm.totalPrice),
              notes: contractForm.projectDescription || 'Complete bathroom renovation as specified'
            }
          ]
        };

        // Save temp quote to backend
        await axios.post(`${API}/quotes/request`, {
          client_info: tempQuote.client_info,
          room_measurements: {
            length: 3.5,
            width: 2.5,
            height: 2.4
            ...
            <CardContent className="p-6 space-y-6">
              ...
              {contractForm.totalPrice && (
                <div className="space-y-4 bg-blue-50 p-4 rounded-lg">
                  <h3 className="font-semibold text-lg text-gray-900">Australian Standard Payment Schedule</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>1. Deposit (10%)</span>
                      <span className="font-semibold">${(parseFloat(contractForm.totalPrice) * 0.10).toLocaleString('en-AU', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>2. Demolition, Frame & Rough-in (40%)</span>
                      <span className="font-semibold">${(parseFloat(contractForm.totalPrice) * 0.40).toLocaleString('en-AU', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>3. Coverings & Tiling (30%)</span>
                      <span className="font-semibold">${(parseFloat(contractForm.totalPrice) * 0.30).toLocaleString('en-AU', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>4. Fit-off & Handover (20%)</span>
                      <span className="font-semibold">${(parseFloat(contractForm.totalPrice) * 0.20).toLocaleString('en-AU', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                    </div>
        ...
        <nav className="p-4 space-y-2">
          ...
          >
            ...
            <div className="text-left">
              ...
              <div className="text-xs text-gray-500">Generate & Manage</div>
            </div>
          </button>
